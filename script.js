window.addEventListener('load', function(){
    ...
    function toggleFullScreen(){
        ...
        if(!document.fullscreenElement) {
            canvas.requestFullscreen().catch(error =>{
                alert(`Error, cant enable full-screen mode: ${error.message}`)
            });
        }
        else {
    ...
    class InputHandler {
        constructor(){
            ...
            window.addEventListener('touchmove', e=>{
                ...
                if(swipDistance < -this.touchThreshold && this.keys.indexOf('swipe up')===-1) 
                this.keys.push('swipe up');
                else if(swipDistance > this.touchThreshold && this.keys.indexOf('swipe down')===-1){
    ...
    class Player{
        ...
        //utility class that return true if player object is on the ground/bottom edge of canvas
        onGround(){
    ...
    function handleEnemies(deltaTime){
        if(enemyTimer > enemyInterval+randomEnemyInterval){
            ...
            randomEnemyInterval = Math.random() * 600;
            enemyTimer= 0;
        }else{
    ...
    let randomEnemyInterval = Math.random() * 600;
